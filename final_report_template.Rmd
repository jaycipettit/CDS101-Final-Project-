...
In this code, we first ensure that the transaction date and transaction time are in the correct formats: transaction_date is converted to a standard Date format (YYYY-MM-DD), and transaction_time is converted to a time format (HH:MM:SS). Next, we create a new dataframe called coffee_sales that contains only the relevant columns for analysis: transaction_time, transaction_qty, product_category, product_type, and product_detail. Finally, we define a function remove_outliers() that identifies outliers using the interquartile range (IQR) method, which considers values outside 1.5 times the IQR above the third quartile or below the first quartile as outliers. Using this function, we filter the original dataset to remove outliers in transaction_qty and unit_price, resulting in a cleaner dataset data_no_outliers that is more suitable for analysis.

```{r}
data$transaction_date <- as.Date(data$transaction_date)

data$transaction_time <- format(strptime(data$transaction_time, "%H:%M:%S"), "%H:%M:%S")
```
